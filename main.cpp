...
public:
    ...
    Organism (Planet const& planet) : home_planet(planet) { //bind cowpisac(wartosczkontruktora)
    }
...
public:
    ...
    this->production_coefficient = (rand()%5+1)/10;
    }

    int add_height(int value){
        height = height+value;
    }
    ...
    if (result){
        cout<<"Food was added to the planet"<<endl;
        return true;
        }
    }
};
...
Organism Jablonka(Earth);
cout << "aaa: "<<Jablonka.get_planet_name() << endl;
...
while(Earth.get_food() && is_at_least_one_animal_alive(head)){
    ...
    List * current = head;
    while(current){
        if (current->animal!=NULL) current->animal->consumeFood();
        if (current->plant!=NULL) {
            current->plant->produceFood();
            current->plant->add_height(1);
        };
    current = current->next;
    }
...
//while(Earth.get_food()) rekin.consumeFood();
